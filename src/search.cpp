...
constexpr int ttq_mainhist_bonus                       = 747;
constexpr int ttq_prev_mainhist_malus                  = 1091;
constexpr int policy_mainhist_bonus                    = 1340;
constexpr int policy_pawnhist_bonus                    = 1159;
constexpr int pcb_capthist_bonus                       = 1226;
constexpr int post_lmr_conthist_bonus                  = 1427;
constexpr int pcb_mainhist_bonus                       = 212;
constexpr int pcb_conthist_bonus                       = 416;
constexpr int pcb_pawnhist_bonus                       = 1073;
constexpr int fail_low_mainhist_bonus                  = 287;
constexpr int stats_update_mainhist_bonus              = 1131;
constexpr int stats_update_mainhist_malus              = 1028;
constexpr int stats_update_capthist_bonus              = 1291;
constexpr int stats_update_capthist_malus              = 1090;
constexpr int stats_update_p_early_move_conthist_malus = 919;
constexpr int quiet_histories_lowplyhist_bonus         = 874;
constexpr int quiet_histories_conthist_bonus           = 853;
constexpr int quiet_histories_pawnhist_bonus           = 628;
constexpr int conthist_bonuses[6]                      = {1024, 571, 339, 500, 0, 592};
constexpr int lowplyhist_fill                          = 106;
constexpr int mainhistory_fill                         = 61;
constexpr int capthist_fill                            = -598;
constexpr int pawnhist_fill                            = -1181;
constexpr int conthist_fill                            = -427;
...
void Search::Worker::iterative_deepening() {
    ...
    lowPlyHistory.fill(lowplyhist_fill);
...
void Search::Worker::clear() {
    mainHistory.fill(mainhistory_fill);
    lowPlyHistory.fill(lowplyhist_fill);
    captureHistory.fill(capthist_fill);
    pawnHistory.fill(pawnhist_fill);
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, ttData.move,
                                       stat_bonus(depth) * ttq_mainhist_bonus / 1024);

            // Extra penalty for early quiet moves of
            // the previous ply (~1 Elo on STC, ~2 Elo on LTC)
            if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 2 && !priorCapture)
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                              -stat_malus(depth + 1) * ttq_prev_mainhist_malus
                                                / 1024);
        }
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()]
          << bonus * policy_mainhist_bonus / 1024;
        if (type_of(pos.piece_on(prevSq)) != PAWN && ((ss - 1)->currentMove).type_of() != PROMOTION)
            thisThread->pawnHistory[pawn_structure_index(pos)][pos.piece_on(prevSq)][prevSq]
              << bonus * policy_pawnhist_bonus / 1024;
    }
...
moves_loop:  // When in check, search starts here
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                update_continuation_histories(ss, movedPiece, move.to_sq(),
                                              bonus * post_lmr_conthist_bonus / 1024);
            }
        }
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      scaledBonus * pcb_conthist_bonus / 1024);

        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()]
          << scaledBonus * pcb_mainhist_bonus / 1024;

        if (type_of(pos.piece_on(prevSq)) != PAWN && ((ss - 1)->currentMove).type_of() != PROMOTION)
            thisThread->pawnHistory[pawn_structure_index(pos)][pos.piece_on(prevSq)][prevSq]
              << scaledBonus * pcb_pawnhist_bonus / 1024;
    }
    ...
    // Bonus when search fails low and there is a TT move
        ...
        thisThread->mainHistory[us][ttData.move.from_to()]
          << stat_bonus(depth) * fail_low_mainhist_bonus / 1024;
...
void update_all_stats(const Position&      pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        update_quiet_histories(pos, ss, workerThread, bestMove,
                               bonus * stats_update_mainhist_bonus / 1024);

        // Decrease stats for all non-best quiet moves
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move,
                                   -malus * stats_update_mainhist_malus / 1024);
    }
    else
    {
        ...
        captureHistory[moved_piece][bestMove.to_sq()][captured]
          << bonus * stats_update_capthist_bonus / 1024;
    }
    ...
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      -malus * stats_update_p_early_move_conthist_malus / 1024);
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[moved_piece][move.to_sq()][captured]
          << -malus * stats_update_capthist_malus / 1024;
    }
}
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    ...
    for (int i : {1, 2, 3, 4, 6})
    {
        ...
        if (((ss - i)->currentMove).is_ok())
            (*(ss - i)->continuationHistory)[pc][to] << bonus * conthist_bonuses[i - 1] / 1024;
    }
}
  ...
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.from_to()]
          << bonus * quiet_histories_lowplyhist_bonus / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(),
                                  bonus * quiet_histories_conthist_bonus / 1024);

    int pIndex = pawn_structure_index(pos);
    workerThread.pawnHistory[pIndex][pos.moved_piece(move)][move.to_sq()]
      << bonus * quiet_histories_pawnhist_bonus / 1024;
}
