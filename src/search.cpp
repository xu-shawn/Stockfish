           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            while (true)
            {
                ...
                bestValue = search<Root>(rootPos, ss, alpha, beta, adjustedDepth, false, 0);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, Depth pvDistance) {
    ...
    if (!ss->ttPv && (pvDistance > 1 || priorReduction > 4) && depth < 14
        ...
        && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        ...
        Value nullValue =
          -search<NonPV>(pos, ss + 1, -beta, -beta + 1, depth - R, false, pvDistance + 1);
        ...
        if (nullValue >= beta && !is_win(nullValue))
        {
            ...
            Value v = search<NonPV>(pos, ss, beta - 1, beta, depth - R, false, pvDistance);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha, d, true, pvDistance + 1);
            ss->reduction = 0;
