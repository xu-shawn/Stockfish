...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos, Stack* ss) {
    ...
    const auto  zcv   = w.zobristCorrectionHistory[us][zobrist_index(pos)];
    int         cntcv = 1;

    if (m.is_ok())
        cntcv = int((*(ss - 2)->continuationCorrectionHistory)[pos.piece_on(m.to_sq())][m.to_sq()]);

    const auto cv = (5532 * pcv + 394 * mcv + 3269 * macv + 5660 * micv + 6237 * (wnpcv + bnpcv)
                     + zcv * 6000 + cntcv * 5555)
                  / 131072;
    v += cv;
...
moves_loop:  // When in check, search starts here
        ...
        && !(!bestMove && bestValue >= ss->staticEval))
    {
        ...
        thisThread->nonPawnCorrectionHistory[BLACK][us][non_pawn_index<BLACK>(pos)]
          << bonus * 165 / 128;
        thisThread->zobristCorrectionHistory[us][zobrist_index(pos)] << bonus * 128 / 128;
