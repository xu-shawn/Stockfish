...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && depth - (std::min(int(eval - beta) / 152, 6) + depth / 3 + 5) > 0
        && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 15246 && eval >= beta
        && ss->staticEval >= beta - 21 * depth + 366 && !excludedMove && pos.non_pawn_material(us)
