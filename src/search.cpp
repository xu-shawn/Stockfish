...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonus = (55 * (depth > 4) + 82 * (depth > 5) + 124 * (PvNode || cutNode)
                     + 155 * ((ss - 1)->statScore < -14144) + 118 * ((ss - 1)->moveCount > 9)
                     + 83 * (!ss->inCheck && bestValue <= ss->staticEval - 115)
                     + 115 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 81));
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
