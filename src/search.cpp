                ...
                && (ttData.bound & BOUND_LOWER) && ttData.depth >= depth - 3)
            {
                ...
                if (value < singularBeta)
                {
                    ...
                    int doubleMargin = 262 * PvNode - 188 * !ttCapture - corrValAdj1
                                     - ttMoveHistory / 128
                                     - (ss->ply * 2 > thisThread->rootDepth * 3) * 50;
                    int tripleMargin = 88 + 265 * PvNode - 256 * !ttCapture + 93 * ss->ttPv
                                     - corrValAdj2 - std::max<std::int16_t>(ttMoveHistory, 0) / 128
                                     - (ss->ply * 2 > thisThread->rootDepth * 3) * 50;

                    extension = 1 + (value < singularBeta - doubleMargin)
                              + (value < singularBeta - tripleMargin);

                    if (value < singularBeta - tripleMargin && !ss->ttPv)
                        extension = std::max(extension, ttData.depth + 1 - depth);
