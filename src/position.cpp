...
void StateInfo::set_check_info() {
    ...
    checkSquares[PAWN]   = attacks_bb<PAWN>(ksq, ~sideToMove);
...
void StateInfo::update_slider_blockers(Color c) {
...
Bitboard StateInfo::attackers_to(Square s, Bitboard occupied) const {
...
bool StateInfo::attackers_to_exist(Square s, Bitboard occupied, Color c) const {
...
bool StateInfo::legal(Move m) const {
...
bool StateInfo::pseudo_legal(const Move m) const {
...
bool StateInfo::gives_check(Move m) const {
...
// will be prefetched
DirtyPiece StateInfo::do_move(Move m, bool givesCheck, const TranspositionTable* tt = nullptr) {
    ...
    else if (captured)
    {
        ...
        {
            nonPawnMaterial[them] -= PieceValue[captured];
            nonPawnKey[them] ^= Zobrist::psq[captured][capsq];
    ...
    if (type_of(pc) == PAWN)
    {
        ...
        else if (m.type_of() == PROMOTION)
        {
            ...
            // Update material
            nonPawnMaterial[us] += PieceValue[promotion];
        }
    ...
    {
        nonPawnKey[us] ^= Zobrist::psq[pc][from] ^ Zobrist::psq[pc][to];
...
void StateInfo::do_null_move(const TranspositionTable& tt) {
...
bool StateInfo::is_50mr_draw() const {
    return rule50_count() > 99 && (!checkers() || MoveList<LEGAL>(*this).size());
}
...
bool StateInfo::see_ge(Move m, int threshold) const {
...
bool StateInfo::pos_is_ok() const {
...
// Returns a FEN representation of the position. In case of
// Chess960 the Shredder-FEN notation is used. This is mainly a debugging function.
string Position::fen() const { return state().fen(); }

// Computes a bitboard of all pieces which attack a given square.
// Slider attacks use the occupied bitboard to indicate occupancy.
Bitboard Position::attackers_to(Square s, Bitboard occupied) const {
    return state().attackers_to(s, occupied);
}

bool Position::attackers_to_exist(Square s, Bitboard occupied, Color c) const {
    return state().attackers_to_exist(s, occupied, c);
}

// Tests whether a pseudo-legal move is legal
bool Position::legal(Move m) const { return state().legal(m); }


// Takes a random move and tests whether the move is
// pseudo-legal. It is used to validate moves from TT that can be corrupted
// due to SMP concurrent access or hash position key aliasing.
bool Position::pseudo_legal(const Move m) const { return state().pseudo_legal(m); }


// Tests whether a pseudo-legal move gives a check
bool Position::gives_check(Move m) const { return state().gives_check(m); }
...
DirtyPiece Position::do_move(Move m, bool givesCheck, const TranspositionTable* tt = nullptr) {
    ...
    sts.back().repetition = 0;
    int end               = std::min(state().rule50_count(), state().plies_from_null());

    if (end >= 4)
    {
        auto stp = sts.crbegin() + 2;
        for (int i = 4; i <= end; i += 2)
        {
            stp += 2;

            if (stp->state.zobrist_key() == state().zobrist_key())
            {
                sts.back().repetition = stp->repetition ? -i : i;
                break;
            }
        }
    }
...
bool Position::is_draw(int ply) const {
    ...
    if (state().rule50_count() > 99 && (!checkers() || MoveList<LEGAL>(*this).size()))
        return true;
...
bool Position::is_repetition(int ply) const { return repetition() && repetition() < ply; }

// Tests whether there has been at least one repetition
// of positions since the last capture or pawn move.
bool Position::has_repeated() const {

    auto stc = sts.crbegin();
    int  end = std::min(state().rule50_count(), state().plies_from_null());
...
bool Position::upcoming_repetition(int ply) const {
    ...
    {
        stp++;
        other ^= stp->state.zobrist_key();
        stp++;
        other ^= stp->state.zobrist_key() ^ Zobrist::side;
