...
std::tuple<bool, TTData, TTWriter> TranspositionTable::probe(const Key key) const {
    ...
    {
        int replaceAge = replace->relative_age(generation8);
        int currentAge = tte[i].relative_age(generation8);

        if (replace->depth8 - replaceAge * replaceAge / 8
            > tte[i].depth8 - currentAge * currentAge / 8)
            replace = &tte[i];
    }
