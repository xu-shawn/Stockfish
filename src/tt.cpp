...
std::tuple<bool, TTData, TTWriter> TranspositionTable::probe(const Key key) const {
    ...
    {
        const auto replaceAge = replace->relative_age(generation8);
        const auto currAge = tte[i].relative_age(generation8);

        if (currAge >= 2 * GENERATION_DELTA)
        {
            if (replaceAge < currAge || (replaceAge == currAge && replace->depth8 < tte[i].depth8))
                replace = &tte[i];
        }
        else if (replace->depth8 - replaceAge > tte[i].depth8 - currAge)
            replace = &tte[i];
    }
