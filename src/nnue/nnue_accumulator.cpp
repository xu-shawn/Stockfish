...
void update_accumulator_incremental_both(const FeatureTransformer& featureTransformer,
                                         Square                    white_ksq,
                                         Square                    black_ksq,
                                         bool                      white_opponent_has_queen,
                                         bool                      black_opponent_has_queen,
                                         AccumulatorState&         target_state,
                                         const AccumulatorState&   computed);
...
void AccumulatorStack::forward_update_incremental_both(const Position&           pos,
    ...
    const bool   white_opponent_has_queen = pos.count<QUEEN>(BLACK) > 0;
    const bool   black_opponent_has_queen = pos.count<QUEEN>(WHITE) > 0;
    ...
    for (usize next = white_begin + 1; next <= shared_begin; ++next)
        update_accumulator_incremental<true>(WHITE, featureTransformer, white_ksq,
                                             white_opponent_has_queen, accumulators[next],
                                             accumulators[next - 1]);
    for (usize next = black_begin + 1; next <= shared_begin; ++next)
        update_accumulator_incremental<true>(BLACK, featureTransformer, black_ksq,
                                             black_opponent_has_queen, accumulators[next],
                                             accumulators[next - 1]);

    for (usize next = shared_begin + 1; next < size; ++next)
        update_accumulator_incremental_both(
          featureTransformer, white_ksq, black_ksq, white_opponent_has_queen,
          black_opponent_has_queen, accumulators[next], accumulators[next - 1]);
...
void apply_combined(Color                              perspective,
    ...
    const auto* auxWeights  = &featureTransformer.auxWeights[0];
    ...
    for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j)
    {
        ...
        // Lambdas capturing acc[] by reference; each applies a list of i8 weight columns
        // to the accumulator registers for the current tile.
        auto apply_sub = [&](const WeightType* base, const auto& indices) {
            for (int i = 0; i < indices.ssize(); ++i)
            {
                const auto* col = reinterpret_cast<const vec_i8_t*>(
                  &base[indices[i] * Dimensions + tileOff]);
    #ifdef USE_NEON
                ...
                for (IndexType k = 0; k < Tiling::NumRegs; k += 2)
                {
                    const __m128i weight = __lsx_vld(reinterpret_cast<const void*>(&col[k]), 0);
        ...
        auto apply_add = [&](const WeightType* base, const auto& indices) {
            for (int i = 0; i < indices.ssize(); ++i)
            {
                const auto* col = reinterpret_cast<const vec_i8_t*>(
                  &base[indices[i] * Dimensions + tileOff]);
        ...
        apply_sub(psqWeights, psqRemoved);
        apply_add(psqWeights, psqAdded);
        apply_sub(auxWeights,  thrRemoved);
        apply_add(auxWeights,  thrAdded);
    ...
    for (IndexType j = 0; j < PSQTBuckets / Tiling::PsqtTileHeight; ++j)
    {
        ...
        {
            auto* columnPsqt = reinterpret_cast<const psqt_vec_t*>(
              &featureTransformer.auxPsqtWeights[thrRemoved[i] * PSQTBuckets + psqtTileOff]);
            for (usize k = 0; k < Tiling::NumPsqtRegs; ++k)
                psqt[k] = vec_sub_psqt_32(psqt[k], columnPsqt[k]);
        }
    ...
    const auto* auxPsqtWeights    = &featureTransformer.auxPsqtWeights[0];
    ...
    while (tileOffset < Dimensions)
    {
        ...
        for (int i : thrRemoved)
            accum = __riscv_vwsub_wv_i16m8(
              accum, __riscv_vle8_v_i8m4(&auxWeights[i * Dimensions + tileOffset], vl), vl);
        for (int i : thrAdded)
            accum = __riscv_vwadd_wv_i16m8(
              accum, __riscv_vle8_v_i8m4(&auxWeights[i * Dimensions + tileOffset], vl), vl);
    ...
    while (tileOffset < PSQTBuckets)
    {
        ...
        for (int i : thrRemoved)
            accum = __riscv_vsub_vv_i32m1(
              accum, __riscv_vle32_v_i32m1(&auxPsqtWeights[i * PSQTBuckets + tileOffset], vl), vl);
        for (int i : thrAdded)
            accum = __riscv_vadd_vv_i32m1(
              accum, __riscv_vle32_v_i32m1(&auxPsqtWeights[i * PSQTBuckets + tileOffset], vl), vl);
...
void update_accumulator_incremental_both(const FeatureTransformer& featureTransformer,
    ...
    const auto* threat_pp_base = &featureTransformer.auxWeights[0];
    ...
    PSQFeatureSet::append_changed_indices(WHITE, white_ksq, target_state.dirtyPiece,
                                          white_opponent_has_queen, psq_removed[WHITE],
                                          psq_added[WHITE]);
    PSQFeatureSet::append_changed_indices(BLACK, black_ksq, target_state.dirtyPiece,
                                          black_opponent_has_queen, psq_removed[BLACK],
                                          psq_added[BLACK]);
    ...
    target_state.opponentQueens = computed.opponentQueens;
...
void update_accumulator_refresh_cache(Color                     perspective,
    ...
    for (IndexType j = 0; j < PSQTBuckets / Tiling::PsqtTileHeight; ++j)
    {
        ...
        {
            auto* columnPsqt = reinterpret_cast<const psqt_vec_t*>(
              &featureTransformer.auxPsqtWeights[active[i] * PSQTBuckets + psqtTileOff]);
            for (usize k = 0; k < Tiling::NumPsqtRegs; ++k)
                psqt[k] = vec_add_psqt_32(psqt[k], columnPsqt[k]);
        }
    ...
    while (tileOffset < PSQTBuckets)
    {
        ...
        for (int i : active)
            accum = __riscv_vadd_vv_i32m1(
              accum, __riscv_vle32_v_i32m1(&auxPsqtWeights[i * PSQTBuckets + tileOffset], vl),
              vl);
    ...
    for (const auto index : active)
    {
        ...
        for (IndexType j = 0; j < Dimensions; ++j)
            accumulator.accumulation[perspective][j] +=
              featureTransformer.auxWeights[offset + j];

        for (usize k = 0; k < PSQTBuckets; ++k)
            accumulator.psqtAccumulation[perspective][k] +=
              featureTransformer.auxPsqtWeights[index * PSQTBuckets + k];
    }
