...
class FeatureTransformer {
   ...
   public:
    ...
    bool read_parameters(std::istream& stream) {
        ...
        {
            read_leb_128<BiasType>(stream, biases.data(), HalfDimensions);
            read_leb_128<WeightType>(stream, weights.data(), HalfDimensions * InputDimensions);
            read_leb_128<PSQTWeightType>(stream, psqtWeights.data(), PSQTBuckets * InputDimensions);
        }
        permute_weights();
        scale_weights(true);
    ...
    std::tuple<std::array<WeightType, HalfDimensions * IndexType::Dimensions>, ...> weights_stack;
    alignas(CacheLineSize) std::array<WeightType, HalfDimensions * InputDimensions> weights;
    alignas(
      CacheLineSize) std::array<WeightType, HalfDimensions * ThreatInputDimensions> threatWeights;
    alignas(CacheLineSize) std::array<PSQTWeightType, InputDimensions * PSQTBuckets> psqtWeights;
    alignas(CacheLineSize)
      std::array<PSQTWeightType, ThreatInputDimensions * PSQTBuckets> threatPsqtWeights;
};
