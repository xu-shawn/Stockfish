...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            m.value += 2 * (*continuationHistory[0])[false][NO_PIECE_TYPE][pc][to];
            m.value += (*continuationHistory[1])[false][NO_PIECE_TYPE][pc][to];
            m.value += (*continuationHistory[2])[false][NO_PIECE_TYPE][pc][to] / 3;
            m.value += (*continuationHistory[3])[false][NO_PIECE_TYPE][pc][to];
            m.value += (*continuationHistory[5])[false][NO_PIECE_TYPE][pc][to];
        ...
        else  // Type == EVASIONS
        {
            if (pos.capture_stage(m))
                ...
                m.value =
                  ...
                  + (*continuationHistory[0])[false][NO_PIECE_TYPE][pos.moved_piece(m)][m.to_sq()]
...
top:
    ...
    case GOOD_CAPTURE :
        if (select<Next>([&]() {
                ...
                return pos.see_ge(*cur, -cur->SEEValue / 18) ? true
