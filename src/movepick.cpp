...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        if constexpr (Type == CAPTURES)
            m.value =
              6773 * int(PieceValue[pos.piece_on(m.to_sq())]) / 1000
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            // histories
            m.value = 1008 * (*mainHistory)[pos.side_to_move()][m.from_to()];
            m.value += 1938 * (*pawnHistory)[pawn_structure_index(pos)][pc][to];
            m.value += 1917 * (*continuationHistory[0])[pc][to];
            m.value += 1033 * (*continuationHistory[1])[pc][to];
            m.value += 358 * (*continuationHistory[2])[pc][to];
            m.value += 988 * (*continuationHistory[3])[pc][to];
            m.value += 1081 * (*continuationHistory[5])[pc][to];

            m.value /= 1024;

            m.value += (m == killer) * 70482;

            // bonus for checks
            m.value += bool(pos.check_squares(pt) & to) * 15403;

            // bonus for escaping from capture
            m.value += threatenedPieces & from ? (pt == QUEEN && !(to & threatenedByRook)   ? 48672
                                                  : pt == ROOK && !(to & threatenedByMinor) ? 26176
                                                  : !(to & threatenedByPawn)                ? 14303
                                                                                            : 0)
                                               : 0;

            // malus for putting piece en prise
            m.value -= (pt == QUEEN  ? bool(to & threatenedByRook) * 49857
                        : pt == ROOK ? bool(to & threatenedByMinor) * 23393
                                     : bool(to & threatenedByPawn) * 15207);
        }
